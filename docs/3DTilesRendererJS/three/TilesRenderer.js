...
import { Matrix4, Vector3, Vector2, Frustum, LoadingManager } from "three";
...
export class TilesRenderer extends TilesRendererBase {
  ...
  update() {
    ...
    if (
      Math.abs(
        Math.max(tempVector.x - tempVector.y, tempVector.x - tempVector.z)
      ) > 1e-6
    ) {
      console.warn(
  ...
  parseTile(buffer, tile, extension) {
    ...
    switch (fileType) {
      ...
      default:
        console.warn(
          `TilesRenderer: Content type "${fileType}" not supported.`
        );
